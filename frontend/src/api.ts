...
export const api = {
  ...
  getAnalysis: (anchorStatementId: string, from?: string, to?: string) => {
    const params = new URLSearchParams({ anchor_statement_id: anchorStatementId })
    if (from) params.set('from', from)
    if (to) params.set('to', to)
    return request<Analysis>(`/v1/analysis?${params.toString()}`)
  },
}
