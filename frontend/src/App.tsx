...
import type { Analysis, Insight, StatementDetail, StatementMetadata } from './api'
...
function InsightsView({ insights }: { insights: Insight[] }) { return <Paper sx={{ p: 2 }}><Typography variant="h5" gutterBottom>Hallazgos del período</Typography>{insights.length === 0 ? <Typography color="text.secondary">No hay hallazgos adicionales para este período.</Typography> : <Stack spacing={2}>{insights.map((insight) => <Alert key={insight.kind} severity={insight.kind.endsWith('warning') ? 'warning' : 'info'}><Typography variant="subtitle2">{insight.priority}. {insight.title}</Typography><Typography variant="body2">{insight.body}</Typography><Box component="ul" sx={{ my: 1, pl: 2.5 }}>{insight.evidence.map((item, index) => <li key={`${item.label}-${index}`}><Typography variant="body2">{evidenceLabel(item.label)}: {evidenceValue(item)}</Typography></li>)}</Box>{insight.caveat && <Typography variant="caption">{insight.caveat}</Typography>}</Alert>)}</Stack>}</Paper> }
...
function evidenceLabel(label: string) { return ({ gasto: 'Gasto', movimientos_de_gasto: 'Movimientos de gasto', creditos: 'Créditos', debitos_excluidos: 'Débitos excluidos', monto: 'Monto', movimientos: 'Movimientos', categoria: 'Categoría', comercio: 'Comercio', cadencia: 'Cadencia', ocurrencias: 'Ocurrencias', desde: 'Desde', hasta: 'Hasta', mes_actual: 'Mes actual', gasto_actual: 'Gasto actual', mes_anterior: 'Mes anterior', gasto_anterior: 'Gasto anterior', cambio_absoluto: 'Cambio absoluto', cambio_porcentual: 'Cambio porcentual', hueco: 'Hueco', mes_parcial: 'Mes parcial', monto_sin_categoria: 'Monto sin categoría', movimientos_sin_categoria: 'Movimientos sin categoría', monto_sin_comercio: 'Monto sin comercio', movimientos_sin_comercio: 'Movimientos sin comercio', versiones_ruleset: 'Versiones de reglas' } as Record<string, string>)[label] ?? label }
function evidenceValue(item: Insight['evidence'][number]) { return ['gasto', 'creditos', 'debitos_excluidos', 'monto', 'gasto_actual', 'gasto_anterior', 'cambio_absoluto', 'monto_sin_categoria', 'monto_sin_comercio'].includes(item.label) && typeof item.value === 'number' ? money.format(item.value) : item.value }
